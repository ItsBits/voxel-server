...
//==============================================================================
constexpr Voxel::u32Vec3 Voxel::CHUNK_POSITION_HASH_SEED;
constexpr Voxel::u32Vec3 Voxel::REGION_POSITION_HASH_SEED;
constexpr Voxel::s32Vec3 Voxel::REGION_SIZE;
constexpr Voxel::s32Vec3 Voxel::CHUNK_SIZE;
...
//==============================================================================
Voxel::Voxel() : c_maximum_compressed_size{ compressBound(CHUNK_VOLUME * sizeof(Block)) } {
...
//==============================================================================
void Voxel::create_new_chunk(Chunk * chunk, const s32Vec3 & chunk_position) {
...
//==============================================================================
void Voxel::set_block(const s32Vec3 & block_position, const Block & b) {
...
//==============================================================================
Voxel::Chunk * Voxel::get_chunk(const s32Vec3 & chunk_position) {
...
//==============================================================================
void Voxel::get_chunk_blocks(const s32Vec3 & chunk_position, Block * c) {
    assert(c != nullptr);
    Chunk * chunk = get_chunk(chunk_position);
    assert(chunk != nullptr);
...
//==============================================================================
const Voxel::Block * Voxel::get_chunk_blocks_reference(const s32Vec3 & chunk_position) {
    Chunk * chunk = get_chunk(chunk_position);
    assert(chunk != nullptr);
    return chunk->blocks;
}
...
//==============================================================================
uint32_t Voxel::chunk_index(const s32Vec3 & chunk_position) {
...
//==============================================================================
uint32_t Voxel::region_index(const s32Vec3 & region_position) {
...
//==============================================================================
void Voxel::close_chunk(Chunk * chunk) {
    assert(chunk != nullptr);
...
//==============================================================================
void Voxel::defragment_region(Region * region) {
    assert(region != nullptr);
...
//==============================================================================
Voxel::Region * Voxel::get_region(const s32Vec3 & region_position) {
...
//==============================================================================
void Voxel::close_region(Region * region) {
    assert(region != nullptr);
...
T * Voxel::LeastRecentlyUsed<T, N>::get_node(const s32Vec3 & node_position, uint32_t node_index) {
...
//==============================================================================
template<typename T, uint32_t N>
void Voxel::LeastRecentlyUsed<T, N>::add_node(T * node, uint32_t index) {
    // add to map
    assert(index < N);
...
//==============================================================================
template<typename T, uint32_t N>
T * Voxel::LeastRecentlyUsed<T, N>::remove_lru_node() {
...
//==============================================================================
template<typename T, uint32_t N>
void Voxel::LeastRecentlyUsed<T, N>::remove_node(T * node) {
    assert(node != nullptr);
...
//==============================================================================
template<typename T, uint32_t N>
Voxel::LeastRecentlyUsed<T, N>::LeastRecentlyUsed() {
...
//==============================================================================
template<typename T, uint32_t N>
void Voxel::LeastRecentlyUsed<T, N>::add_to_heap(T * node) {
    assert(node != nullptr);
...
//==============================================================================
template<typename T, uint32_t N>
T * Voxel::LeastRecentlyUsed<T, N>::get_from_heap() {
    T * node = m_free_list;
    if (node != nullptr)
...
//==============================================================================
void Voxel::Region::read(void * buffer, size_t count, size_t position) {
    pread(fd, buffer, count, position);
}

//==============================================================================
void Voxel::Region::write(const void * buffer, size_t count, size_t position) {
    pwrite(fd, buffer, count, position);
}

//==============================================================================
bool Voxel::Region::open_file(const char * const file_name) {
